...
3. **For ML/AI libraries and research artifacts**, invoke the `huggingface-skills:huggingface-papers` skill for HF paper pages (markdown + linked models/datasets/spaces) and the `huggingface-skills:hf-cli` skill for model cards, dataset cards, and hub metadata. These are primary sources for anything Hugging Face-adjacent.
