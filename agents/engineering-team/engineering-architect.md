...
- **Default to a clean client/server (or producer/consumer) split** with one explicit, named boundary contract over a blurred in-process tangle. A named boundary is independently testable and lets two agents work the two sides without colliding — it is what let recent features be proven on real repos cleanly. Reach for the split whenever a unit both produces and consumes across a process, network, or trust line.
- **Fail loud; never silently degrade.** Every error path must surface — a typed error, a non-zero exit, or a visible log line — or be a documented, evaluated exception. A silent fallback that swallows a failed write or a full disk turns a clean, diagnosable error into a mystery stall. Design errors to be seen, and seed a fail-loud criterion into the security/maintainability evals you co-own.
